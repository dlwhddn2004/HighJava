...
public class T06_ListSortTest {
	 ...
	 * 	 정렬 기준을 별도로 구현하고 싶을 때는 Comparator 구현하여 사용하면 된다.
	 * 
	...
	public static void main(String[] args) {
		...
		System.out.println("정렬 되기 전 list : " +list);
		
		//정렬은 Colletions.sort()메서드를 이용하여 정렬한다.
		//정렬은 기본적으로 '오름차순 정렬'을 수행한다.
		//정렬방식을 변경하려면 정렬방식을 결정하는 객체를 만들어서
		//Colletions.sort()메서드에 인수로 넘겨주면 된다.
		
		//Colletion + s<= s 는 colletion인터페이스의 클레스 , static 메소드 :sort
 ...
 *  Comparator인터페이스의 compare()라는 메서드를 재정의하여 구현하면 된다.
 *
 */
class Desc implements Comparator<String>{
/*
 * compare()메서드의 반환값을 결정하는 방법
 * 1. compare() 메서드가 양수를 반환하면 두값의 순서가 바뀐다.(오름차순이 기본)
 * 
 * 	  오름차순 정렬일 경우
 * 	  => 앞의 값이 크면 양수, 같으면 0, 앞의 값이 작으면 음수를 반환하도록 한다. 
 * 
 * 	-String객체에는 정렬을 위해서 compareTO()메서드가 구현되어 있는데
 * 	이 메서드의 반환값은 오름차순에 맞게 반한되도록 구현되어 있다.
 ...
 */
	...
	public int compare(String str1, String str2) {
		...
		/*return str1.compareTo(str2) * -1; 할경우 내림차순*/
	}
