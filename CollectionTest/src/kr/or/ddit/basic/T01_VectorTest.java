...
public class T01_VectorTest {
	public static void main(String[] args) {
		...
		//Vector는 add()메서드를 이용하여 데이터를 추가 할 수 있다.
		v1.add("aaa");
		v1.add(111);
		...
		//Vector는 addElement메서드를 이용하여 추가 할 수도 있는데
		//이 메서드는 기본적으로 add()메서드와 같은 기능을 수행한다.
		//v1.addElement("ccc"); << Vetor 로해야함 List엔 없는 메소드
		...
		//				=> 자료가 삭제되면 index번째 이후의 데이터들이 앞으로 자동 당겨져서 채워짐.
		//				=> 반환값 : 삭제된 데이터
		
		
		//remove(삭제할 데이터) => '삭제할 데이터'를 찾아 삭제한다
		//					    => 만약 '삭제할 데이터'가 여러개이면 앞에서부터 삭제한다. 
		//							ex3) 홍길동이 4개 일경우 가장 앞에있는 홍길동만 삭제 = 맨앞 홍길동 삭제 , 남은 3개 홍길동 생존)
		//						=> 삭제할 데이터가 '정수형'이거나 'char'형 일경우는 
		//					 		삭제할 데이터를 객체로 변환해서 사용해야 한다.(아니면 인덱스로 인식)
		...
		v1.remove(new Integer(123)); //인덱스로 오해할 상황이 생기니 객체로 만들어주고 삭제
		System.out.println("삭제 후 : "+v1);
		System.out.println();
		...
		/*
		 *  제너릭 타입(generic type)
		 *  =>Colletion객체를 선언할 때 <> 안에 그 Colletion이 저장할 데이터 타입을 정해주는
		 *  것을 말한다. 
		 ...
		 *  제너릭 타입으로 선언하게 되면 데이터를 꺼내올 때 별도의 형변환이 필요 없다.
		 */
		Vector<String> v2 = new Vector<String>();
		Vector<Integer> v3 = new Vector<Integer>();
		
		v2.add("안녕하세요");
		//v2.add(123); //오류 : 다른 종류의 데이터를 추가 할 수 없다.
		...
		//removeAll(Collection 객체) => 벡터의 데이터중에서 'Collection객체'가 
		// 가지고 있는 데이터를 모두 삭제한다.
		v2.removeAll(v4);
		...
		/*
		 * 	향상된 for문
		 *  형식) for(자료형명 변수명 : 배열변수나 Collection 계열 번수){
		 *  처리할 내용들;
		 *  ...
		 *  }
		 *  
		 *  => 주어진'배열변수나 Collection 계열 변수'의 데이터 개수 만큼 반복한다.
		...
		for(String s : v2) {
			...
			//만약 제너릭을 사용하지 않은 collection을 향상된 for문으로 처리 할 경우
			//에는 Object형으로 간주한다.
			for(Object obj : v1) {
			...
			v.trimToSize(); //백터의 용량을 현제 벡터 사이즈로 줄인다.
			...
			v.setSize(7); //현재 용량이 설정 사이즈보다 작으면 신규용량 = 현재용량 * 2
						  //그래도 설정이 사이즈보다 작다면 신규용량 = 설정사이즈로 설정된다.
